...
Page({
  ...
  exitGrid:function(event){
     //xml页面中给标签加上一个ID就能得到
     //<view class="item orange"                         bindtap="exitGrid" id="id1">
      //console.log(event.target.id);//event是一个对象
        // var data=this.data.showdData+event. target.id;
 console.log(event.target.id);
    ...
    if(id == this.data.id1){//如果点击的是退格键，则执行如下操作
      ...
      if(data == 0){//如果值是0，不执行任何操作
        ...
        this.data.arr.pop();//退格删除最后一个数据
      }
    }else if(id == this.data.id2){//如果点击的是清屏操作如下
      this.setData({screenData:"0"});
      this.data.arr.length = 0;//退格清空数组数据
      ...
      if(firstWord == "-"){//得到第一个值是- ，重新赋值
        ...
        this.data.arr.shift();//去掉第一个元素
      }else{
        data = "-"+data;
        this.data.arr.unshift("-");//在第一个元素增加一个负号
      }
      this.setData({screenData:data});
